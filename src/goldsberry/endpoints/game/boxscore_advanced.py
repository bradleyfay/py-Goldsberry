...
class BoxscoreAdvancedEndpoint:
    """Fetch advanced boxscore statistics for a specific game.

    Provides access to the boxscoreadvancedv2 endpoint which returns
    player and team advanced statistics including offensive/defensive ratings,
    usage percentage, true shooting, and other efficiency metrics.
    ...
    Example:
        ...
        >>> top_performers = boxscore.get_top_performers_by_pie(limit=3)
        >>> for player in top_performers:
        ...     print(f"{player.player_name}: PIE {player.pie:.3f}")
    ...
    ENDPOINT = "boxscoreadvancedv2"  # NBA API endpoint name
    ...
    def _parse_response(self, data: dict) -> BoxscoreAdvanced:
        """Parse NBA API response into BoxscoreAdvanced model.

        Args:
            data: Raw API response

        Returns:
            BoxscoreAdvanced with player and team stats
        """
        result_sets = data.get("resultSets", [])
        if not result_sets:
            return BoxscoreAdvanced(player_stats=[], team_stats=[])
        ...
        if len(result_sets) > 0:
            ...
            for row in rows:
                ...
                player_stats.append(PlayerBoxscoreAdvanced(**player_dict))
        ...
        if len(result_sets) > 1:
            ...
            for row in rows:
                ...
                team_stats.append(TeamBoxscoreAdvanced(**team_dict))

        return BoxscoreAdvanced(player_stats=player_stats, team_stats=team_stats)
    ...
    def fetch(
        ...
        """Fetch advanced boxscore (synchronous).
        ...
        Example:
            ...
            >>> top_performers = boxscore.get_top_performers_by_pie(limit=5)
        """
    ...
    async def fetch_async(
        ...
        """Fetch advanced boxscore (asynchronous).
...
def get_boxscore_advanced(
    ...
    """Convenience function to fetch advanced boxscore.
    ...
    Example:
        >>> from goldsberry.endpoints.game import get_boxscore_advanced
        >>> boxscore = get_boxscore_advanced(game_id="0022400001")
        >>> for player in boxscore.get_top_performers_by_pie(limit=5):
        ...     print(f"{player.player_name}: PIE {player.pie:.3f}")
    """
    endpoint = BoxscoreAdvancedEndpoint()
...
__all__ = ["BoxscoreAdvancedEndpoint", "get_boxscore_advanced"]
